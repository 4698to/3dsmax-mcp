...
2. **dismiss**：关掉上次误点「选定」留下的警告窗（否则会挡住主界面）
3. **cleanup**（若脏）：出现 `模型：N`、或「在场景中选择模型/关节」下方有名称时，点两行「清空」
4. **list slot（必做）**：先点「(选中后在编辑区添加)」（或已有 `模型：N`）；**不点就点选定会弹警告**
5. **mesh**：Max 选中模型 →「选定」→ OCR 校验 `模型：N>=1`
6. **bones**：再点列表行 → Max 选中骨骼 →「选定」→ 校验 `关节：N>=1`
7. **确认**：返回模型名/数量、关节名/数量摘要；**默认不点「开始蒙皮」**
8. **start**：用户确认后调用 `goskin_confirm_start(user_confirmed=true)` 才点击并等待「完成」
