...
- Scene reset on acquire is **opt-in** (`reset_scene=true` or `MAXMCP_RESET_ON_ACQUIRE=true`, default false). When requested, the scene is **saved first** then cleared. Prefer `manage_scene(action="reset")` for an explicit user reset.
