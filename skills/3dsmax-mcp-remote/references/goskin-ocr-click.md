...
3. 用户确认 `goskin_run_skin` 摘要前，**绝不**点「开始蒙皮」。
4. 先聚焦列表槽位「(选中后在编辑区添加)」再点「选定」。
5. 用 OCR 计数（`模型：N` / `关节：N`）校验，不单信点击 `ok`。
6. 场景选择为空时禁止「选定」。
7. Max 所在桌面须 **未锁定**（锁屏 / RDP 断连 → 空成功点击）。
...
| 打开 UI | `goskin_ensure_ready` | 全局蒙皮页就绪 |
| 收集 | `get_unhidden_meshes_bones` | handles 非空 |
| 准备 | `goskin_run_skin` | `awaiting_start_confirm`，模型/关节≥1 |
| 开始 | `goskin_confirm_start(user_confirmed=true)` | OCR「完成」 |

优先传 `mesh_handles` / `bone_handles`。`goskin_run_auto` 默认仍在开始前暂停。

服务端内部顺序（勿自行重写）：dismiss 警告 → cleanup → 槽位 → 选定模型 → 选定关节 → **停止等确认**。
