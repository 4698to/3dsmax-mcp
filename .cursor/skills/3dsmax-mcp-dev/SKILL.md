...
- `list_instances` probes **one Max at a time** under a process-wide lock shared with `list_max_instances` (TCP ping + local named-pipe checks never overlap). Each row reports `tcp_online` and `native_online` (local named pipe only; remote hosts are TCP-only). `online` is true if either transport is up. `acquire_instance` is lock-only and does **not** ping Max. Never call these discovery tools in parallel with each other or with scene tools — Max is single-threaded.
- **Release every 2 rounds:** Every 2 rounds of max-mcp usage (every second completed user request), proactively ask the user "本轮 3ds Max 操作已完成，是否释放当前实例进入空闲待机供他人使用？" — if the user confirms or explicitly indicates the round is done, call `release_instance`; otherwise keep the current lock (the lock has a 30-minute active-renewal TTL safety net, so long tasks are not harmed).
- `release_instance` can only be called by the current holder; session disconnect also auto-releases the lock.
- Single-instance fallback: explicit `acquire_instance` is not required, but the same every-2-rounds release check is recommended.
...
- Reuse `dialog_monitor/_test_goskin_*.py` runners; do not invent new temp smoke scripts when one already covers the case.
...
### Scene management
- `manage_scene` (hold/fetch/reset/save/info/save_older) — TCP and `save_older` call `MCP_SceneManage`
...
- **Download:** `workspace_download(file_name)` returns `data_b64` for files
  **inside the workspace directory only**. Max-generated captures are written to the
  workspace's parent (e.g. `...\Temp\3dsmax-mcp\viewport_*.png`), so a bare download
  fails with `FileNotFoundError` / `NOT_FOUND`. Fix: run `execute_maxscript` with a
  `copyfile` command to copy the file into the workspace directory, then download it.
